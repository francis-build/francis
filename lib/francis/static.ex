defmodule Francis.Static do
  ...
  This module provides functions to resolve asset paths and URLs,
  including support for digested assets generated by `mix francis.digest`.

  ## Usage

      # Get the path for an asset (with digest if available)
      Francis.Static.asset_path("app.css")
      # => "app-a1b2c3d4.css"

      # Check if an asset exists
      Francis.Static.asset_exists?("app.css")
      # => true
  """
  ...
  ## Examples
      ...
      "app-a1b2c3d4.css"
  """
  def static_path(logical_path, static_path \\ "/", manifest_path \\ @default_manifest_path) do
    case load_manifest(manifest_path) do
      {:ok, manifest} ->
        digested_path = get_in(manifest, ["files", logical_path, "digested_path"]) || logical_path

        Path.join(static_path, digested_path)

      _ ->
        Path.join(static_path, logical_path)
    end
  end
